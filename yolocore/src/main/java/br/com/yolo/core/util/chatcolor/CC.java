...
public enum CC {
    ...
    private static final Pattern STRIP_COLOR_PATTERN = Pattern.compile("(?i)" + COLOR_CHAR
            + "[0-9A-FK-OR]");
    ...
    private final static Map<Integer, CC> BY_ID = new HashMap<>();
    private final static Map<Character, CC> BY_CHAR = new HashMap<>();

    CC(char code, int intCode) {
        this(code, intCode, false);
    }

    CC(char code, int intCode, boolean isFormat) {
        this.code = code;
     ...
     * @return Associative {@link CC} with the given code,
